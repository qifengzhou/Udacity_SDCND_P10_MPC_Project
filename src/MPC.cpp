...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // initiate weight for the costs
    double cte_w = 5000;
    double epsi_w = 5000;
    double v_w = 1;
    double delta_w = 5;
    double a_w = 5;
    double delta_d_w = 100;
    double a_d_w = 20;
    ...
    for (int i = 0; i < N -2; i++) {
        fg[0] += delta_d_w * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
        fg[0] += a_d_w * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }

    //the cost is located at 0 in fg, so add 1 to the rest, the constraints in the vector
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //set lower and upper bounds for steering angle
  for (int i = delta_start; i < a_start; i++) {
      vars_lowerbound[i] = -0.436332;
      vars_upperbound[i] = 0.436332;
  }

  //set lower and upper bounds for acceleration
